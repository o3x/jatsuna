...
## [7.2.0] - Sun Feb 22 11:05:00 JST 2026
### Added
- **AI難易度の差別化強化**: 各レベルの実力差を明確化。
  - **Easy**: 明確な「接待モード」へ調整。角を避け、獲得数を抑える。
  - **Medium**: 標準的な貪欲法。
  - **Hard**: Minimax探索（深度3）と戦略的位置評価を導入。
  - **SuperHard**: 探索深度を4に強化し、評価関数を精緻化。
  - **Ultimate**: 既存の結託ロジックをさらに強化。
- **UI実力差ガイド**: 難易度選択時に、各AIの思考特徴を説明するキャプション表示を追加。

## [7.1.6] - Wed Feb 18 10:45:00 JST 2026
### Added
- 石のスタイル定義を `constants.js` に共通化し、一貫性を向上。
### Changed
- `RULEBOOK.md` をゲームアプリの解説から「蛇突奈」という遊びの公式ルールブックとして全面的に刷新。
### Fixed
- ゲーム上部の石数表示で使用されている色が古い設定のままだった問題を修正。
- ゲーム上部の表示がバリアフリーモードの設定と連動していなかった問題を修正。
- `RULEBOOK.md` 内の三すくみ変換ルールの誤記（全て自分の色になるとの記述）を修正。

## [7.1.5]
**日時**: Tue Feb 17 21:55:00 JST 2026  
**変更内容**:

### Fixed
- **統計画面のクラッシュ完全解決（最終安定化）**: `StatsDashboard.jsx` を極限まで簡略化した描画ロジックで再実装。
- **データ連携の安定化**: `App.jsx` での読み込み処理を `useEffect` に一本化し、状態更新の競合を排除。
- **UI 最適化**: 描画エラーのリスクとなる複雑なスタイルや未定義クラスを完全に除去。

## [7.1.1]
**日時**: Tue Feb 17 21:26:00 JST 2026  
**変更内容**:
...
## [7.1.0]
**日時**: Tue Feb 17 21:24:00 JST 2026  
**変更内容**:
...
## [7.0.0]
**日時**: Tue Feb 17 21:22:00 JST 2026  
**変更内容**:
