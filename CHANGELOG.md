...
- **中央壁(X)の復活**: シミュレーションデータに基づきゲームバランスを大幅改善
  ...
  - **効果**: 手番勝率スプレッド 37.6% → 16.3%に改善
  - **色別勝率差**: 4.0% → 0.5%に改善

### Added
- **ゲームバランス検証基盤**: `docs/balance/` に検証データを蓄積する仕組みを構築
  - `scripts/simulate_v3.js`: ランダムノイズ+手番ローテーション対応シミュレーター
  - 1000ゲーム×2条件の検証レポートを保存（JSON+Markdown）
  - 実施者・日時・条件・結果を全て記録し、将来の判断資料に
